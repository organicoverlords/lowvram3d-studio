...
def _studio_tool(
    ...
    command = [
        engine.python,
        str(engine.package_root / "workers" / "studio_meshtools.py"),
        "--url", engine.config.meshtools_url,
        "--operation", operation,
        "--input", str(source),
        "--output", str(output),
        "--report", str(report),
        "--options-json", json.dumps(options, separators=(",", ":")),
        "--timeout", str(timeout),
    ]
    _reusable_command(engine, stage, command, {"mesh": output, "report": report}, receipt, job_dir, timeout)


def _single_part(parts_manifest: Path) -> bool:
    try:
        return int(_read_json(parts_manifest).get("part_count", 0)) == 1
    except Exception:
        return False


def _blender_optimize(
    engine: "PipelineEngine",
    ...
    ratios = ",".join(str(value) for value in profile.lod_ratios)
    artifacts = {"mesh": output, "stats": report}
    ...
    engine._reusable_blender_stage(
        ...
        [
            ...
            "--target-triangles", profile.target_triangles,
            "--target-min", profile.target_min,
            "--target-max", profile.target_max,
            "--budget-mode", profile.budget_mode,
            "--per-object-target", profile.per_object_target,
            "--planar-angle-deg", profile.planar_angle_deg,
            "--lod1", lod1 if profile.lod_count > 0 else "",
            "--lod2", lod2 if profile.lod_count > 1 else "",
            "--lod-ratios", ratios,
            "--lod-count", profile.lod_count,
        ],
        artifacts,
        receipt,
        job_dir,
...
def _blender_uv(
    ...
    atlas_mode = "preserve_or_per_object" if preserve_existing else profile.atlas_mode
    args: list[object] = [
        "--input", source,
        "--output", output,
        "--report", report,
...
def run_postprocess(
    ...
    *,
    ...
    profile = get_profile(asset_type, quality, texture_resolution, lod_enabled, prompt=prompt, filename=mesh.name)
    if resume_job_id:
    ...
    if resume_job_id and "postprocess_contract" not in receipt.parameters and "profile" in receipt.parameters:
        legacy_parameters = {
            ...
            for key in ("profile", "separate_movable_parts", "remove_hidden_geometry", "experimental_semantic_split", "prompt")
        }
        current_legacy_shape = {
            key: request_parameters.get(key)
            for key in legacy_parameters
        }
    ...
    if source_image:
        receipt.input_files["source_image"] = str(source_image)
    ...
    high_glb = job_dir / "source" / "high.glb"
    ingest_report = job_dir / "reports" / "ingest.json"
    engine._reusable_blender_stage(
        "ingest_validate", "ingest_validate.py",
        ["--input", source_original, "--output", high_glb, "--report", ingest_report, "--apply-transforms"],
        {"high_glb": high_glb, "report": ingest_report}, receipt, job_dir,
    )

    analysis_report = job_dir / "reports" / "analysis.json"
    engine._reusable_blender_stage(
        "analyse", "analyze_asset.py",
        ["--input", high_glb, "--report", analysis_report, "--asset-type", profile.asset_type.value],
        {"report": analysis_report}, receipt, job_dir,
    )
    ...
    split_args: list[object] = [
        "--input", high_glb, "--output", split_mesh, "--parts-dir", parts_dir,
        "--manifest", parts_manifest, "--asset-type", profile.asset_type.value,
        "--max-parts", profile.max_parts,
    ]
    if profile.merge_small_fragments:
        split_args.append("--merge-small")
    ...
    engine._reusable_blender_stage(
        "split", "split_parts.py", split_args,
        {"mesh": split_mesh, "manifest": parts_manifest}, receipt, job_dir,
    )

    optimized_mesh = job_dir / "optimized" / "game_ready_untextured.glb"
    optimize_report = job_dir / "reports" / "optimize.json"
    lod1 = job_dir / "optimized" / "lod1.glb"
    lod2 = job_dir / "optimized" / "lod2.glb"
    studio_retopo = job_dir / "optimized" / "studio_retopo.glb"
    studio_retopo_report = job_dir / "reports" / "studio_retopo.json"
    retopo_source = split_mesh
    retopo_backend = "blender"
    if profile.studio_retopo_mode == "single_object" and _single_part(parts_manifest):
        try:
            _studio_tool(
                engine, "retopologize_studio", "auto-retopo", split_mesh,
                studio_retopo, studio_retopo_report, profile.retopo_options,
                receipt, job_dir,
            )
            retopo_source = studio_retopo
            retopo_backend = "3d_gen_studio"
        except StageFailure as exc:
            engine._record_stage_failure(receipt, job_dir, exc.receipt)
            receipt.status = "running"
            receipt.error = None
            engine._write_receipt(receipt, job_dir)
    _blender_optimize(
        engine, profile, retopo_source, optimized_mesh, optimize_report,
        lod1, lod2, receipt, job_dir,
        "retopologize_finalize" if retopo_backend == "3d_gen_studio" else "retopologize_blender",
    )

    uv_mesh = job_dir / "uv" / "game_ready_uv.glb"
    uv_report = job_dir / "reports" / "uv.json"
    uv_layout = job_dir / "uv" / "uv_layout.png"
    studio_uv_mesh = job_dir / "uv" / "studio_uv.glb"
    studio_uv_report = job_dir / "reports" / "studio_uv.json"
    uv_backend = "blender"
    if profile.studio_uv_mode == "single_object" and _single_part(parts_manifest):
        try:
            _studio_tool(
                engine, "uv_studio", "auto-uv", optimized_mesh,
                studio_uv_mesh, studio_uv_report, profile.uv_options,
                receipt, job_dir,
            )
            _blender_uv(
                engine, profile, studio_uv_mesh, uv_mesh, uv_report, uv_layout,
                receipt, job_dir, "uv_finalize", preserve_existing=True,
            )
            uv_backend = "3d_gen_studio"
        except StageFailure as exc:
            engine._record_stage_failure(receipt, job_dir, exc.receipt)
            receipt.status = "running"
            receipt.error = None
    ...
    if uv_backend == "blender":
        _blender_uv(engine, profile, optimized_mesh, uv_mesh, uv_report, uv_layout, receipt, job_dir, "uv_blender")
    ...
    if source_reference:
        appearance_mesh, projected_basecolor, texture_lane = resolve_source_appearance(
            engine, uv_mesh, source_reference, prompt, receipt, job_dir,
            profile.texture_size, profile.uv_padding_px,
        )
    ...
    if profile.texture_strategy == "preserve_existing" and not source_reference:
        engine._reusable_blender_stage(
            "preserve_materials", "preserve_materials.py",
            ["--input", uv_mesh, "--output", baked_mesh, "--maps-dir", maps_dir, "--report", bake_report],
            {"mesh": baked_mesh, "report": bake_report, "maps_manifest": maps_dir / "maps_manifest.json"},
            receipt, job_dir,
        )
    else:
        bake_args: list[object] = [
            "--high", high_glb, "--low", appearance_mesh, "--output", baked_mesh,
            "--maps-dir", maps_dir, "--report", bake_report,
            "--size", profile.texture_size, "--padding-px", profile.uv_padding_px,
        ]
        if projected_basecolor:
            bake_args += ["--basecolor-image", projected_basecolor]
        engine._reusable_blender_stage(
            "bake", "bake_transfer.py", bake_args,
            {
                "mesh": baked_mesh, "report": bake_report,
                "basecolor": maps_dir / "basecolor.png", "normal": maps_dir / "normal.png",
                "roughness": maps_dir / "roughness.png", "metallic": maps_dir / "metallic.png",
                "ao": maps_dir / "ao.png",
            },
            receipt, job_dir, timeout=14_400,
        )
    ...
    if profile.generate_rig:
        ...
        engine._reusable_blender_stage(
            "rig", "rig_animate.py",
            [
                "--input", baked_mesh, "--output", rigged_mesh,
                "--report", rig_report, "--kind", rig_kind, "--prompt", prompt,
    ...
    safe_name = "".join(char if char.isalnum() or char in "-_" else "_" for char in mesh.stem).strip("_") or "asset"
    ...
    engine._reusable_blender_stage(
        "export_validate", "package_validate.py",
        [
            "--input", package_input, "--high", high_glb,
            "--lod1", lod1 if profile.lod_count > 0 else "",
            "--lod2", lod2 if profile.lod_count > 1 else "",
            "--maps-dir", maps_dir, "--parts-manifest", parts_manifest,
            "--uv-layout", uv_layout, "--stage-reports-dir", job_dir / "reports",
            "--output-root", output_root, "--validation", validation,
            "--pipeline-report", pipeline_report, "--asset-type", profile.asset_type.value,
            "--target-min", profile.target_min, "--target-max", profile.target_max,
            "--budget-mode", profile.budget_mode, "--collision-mode", profile.collision_mode,
            "--spatial-chunking", str(profile.spatial_chunking).lower(),
            "--cell-divisions", profile.cell_divisions, "--export-strategy", profile.export_strategy,
        ],
        {
            "glb": output_root / "meshes" / "game_ready.glb",
            "validation": validation, "pipeline_report": pipeline_report,
    ...
    receipt.outputs.update(
        {
            ...
            "validation_json": str(validation), "pipeline_report_json": str(pipeline_report),
            "preview_png": str(output_root / "previews" / "perspective.png"),
            "output_root": str(output_root), "retopo_backend": retopo_backend, "uv_backend": uv_backend,
