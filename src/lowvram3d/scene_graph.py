...
def stable_region_id(region: Mapping[str, Any], index: int) -> str:
    layer = str(region.get("layer_type") or region.get("layer") or region.get("semantic_class") or "unknown").lower()
    prefix = "unknown"
...
def build_scene_graph(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for index, region in enumerate(spec.get("regions", [])):
        ...
        nodes.append({
            "id": generated_id,
            "source_id": source_id,
            "semantic_class": str(region.get("semantic_class") or region.get("layer_type") or region.get("layer") or "unknown"),
