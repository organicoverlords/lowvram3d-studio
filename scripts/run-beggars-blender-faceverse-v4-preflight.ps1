...
$output = "C:\AI\LowVRAM3D-benchmarks\beggars-scene\blender-faceverse-v4-preflight-$runId"
...
$sequenceReport = Join-Path $output 'faceverse_v2_sequence_report.json'
...
foreach ($required in @($python, $source, $model, $checkpoint, $coefficients, $blender)) {
    if (-not (Test-Path -LiteralPath $required)) {
        throw "Blender v4 input missing: $required"
    }
}
...
Invoke-NativeChecked -FilePath $python -ArgumentList @(
    ...
    "import torch; assert torch.__version__.startswith('2.5.1'), torch.__version__; assert torch.cuda.is_available(); print('BLENDER_V4_FACEVERSE_RUNTIME=PROVEN',torch.__version__,torch.cuda.get_device_name(0))"
) -FailureMessage 'Blender v4 FaceVerse runtime validation failed'

Invoke-NativeChecked -FilePath $python -ArgumentList @(
    '-m','py_compile',
    'tools\beggars_scene\export_faceverse_v4_blender_sequence.py'
) -FailureMessage 'Blender v4 sequence exporter failed compilation'
...
try {
    ...
    try {
        ...
        Invoke-NativeChecked -FilePath $python -ArgumentList @(
            ...
            '--anchors','21,31,35',
            '--device','auto'
        ) -FailureMessage 'Blender v4 sequence export failed'
    }
    finally {
    ...
    if (-not (Test-Path -LiteralPath $sequence) -or (Get-Item -LiteralPath $sequence).Length -lt 100000) {
        throw 'Blender v4 sequence is missing or implausibly small'
    }

    Invoke-NativeChecked -FilePath $blender -ArgumentList @(
        '--background',
        '--factory-startup',
        '--python','blender\build_beggars_meme_scene_faceverse_v4.py',
        '--',
        '--sequence',$sequence,
        '--config','configs\scene\beggars_banquet_recreation_v1.json',
        '--output-dir',$output,
        '--render-engine','eevee'
    ) -FailureMessage 'Groomed Blender v4 preflight failed'
    ...
    foreach ($name in @(
        ...
        if (-not (Test-Path -LiteralPath $path) -or (Get-Item -LiteralPath $path).Length -le 0) {
            throw "Groomed Blender v4 output missing or empty: $name"
        }
    }
    Write-Host 'BLENDER_FACEVERSE_V4_SAVE_RELOAD_STILLS=PROVEN'
    ...
    $sheetLines = @(
        ...
        'import os',
        'root = Path(os.environ["B4_OUTPUT"])',
    ...
    $env:B4_OUTPUT = $output
    Invoke-NativeChecked -FilePath $python -ArgumentList @($sheetScript) -FailureMessage 'Could not build Blender v4 contact sheet'
    ...
    if (-not (Test-Path -LiteralPath $contactSheet) -or (Get-Item -LiteralPath $contactSheet).Length -lt 100000) {
        throw 'Blender v4 contact sheet is missing or implausibly small'
    }
}
finally {
...
if ($env:GITHUB_ENV) {
    "B4_OUTPUT=$output" | Add-Content -LiteralPath $env:GITHUB_ENV
}
Write-Host "BLENDER_FACEVERSE_V4_OUTPUT=$output"
exit 0
