...
$output = "C:\AI\LowVRAM3D-benchmarks\beggars-scene\blender-faceverse-v8-preflight-$runId"
...
foreach ($required in @($python, $source, $model, $checkpoint, $coefficients, $blender)) {
    if (-not (Test-Path -LiteralPath $required)) {
        throw "Blender v8 input missing: $required"
    }
}
...
Invoke-NativeChecked -FilePath $python -ArgumentList @(
    ...
    "import torch; assert torch.__version__.startswith('2.5.1'), torch.__version__; assert torch.cuda.is_available(); print('BLENDER_V8_FACEVERSE_RUNTIME=PROVEN',torch.__version__,torch.cuda.get_device_name(0))"
) -FailureMessage 'Blender v8 FaceVerse runtime validation failed'

Invoke-NativeChecked -FilePath $python -ArgumentList @(
    '-m','py_compile',
    'tools\beggars_scene\export_faceverse_v4_blender_model_sequence.py',
    'blender\build_beggars_meme_scene_faceverse_v8.py'
) -FailureMessage 'Blender v8 sources failed compilation'
...
try {
    ...
    try {
        ...
        Invoke-NativeChecked -FilePath $python -ArgumentList @(
            ...
            '--anchors','21,31,35',
            '--device','auto'
        ) -FailureMessage 'Blender v8 model-space sequence export failed'
    }
    finally {
    ...
    if (-not (Test-Path -LiteralPath $sequence) -or (Get-Item -LiteralPath $sequence).Length -lt 100000) {
        throw 'Blender v8 model-space sequence is missing or implausibly small'
    }

    Invoke-NativeChecked -FilePath $blender -ArgumentList @(
        '--background',
        '--factory-startup',
        '--python','blender\build_beggars_meme_scene_faceverse_v8.py',
        '--',
        '--sequence',$sequence,
        '--config','configs\scene\beggars_banquet_recreation_v1.json',
        '--output-dir',$output,
        '--render-engine','eevee'
    ) -FailureMessage 'Anatomically corrected Blender v8 preflight failed'
    ...
    foreach ($name in @(
        ...
        if (-not (Test-Path -LiteralPath $path) -or (Get-Item -LiteralPath $path).Length -le 0) {
            throw "Anatomically corrected Blender v8 output missing or empty: $name"
        }
    }
    Write-Host 'BLENDER_FACEVERSE_V8_SAVE_RELOAD_STILLS=PROVEN'
    ...
    $sheetLines = @(
        ...
        'import os',
        'root = Path(os.environ["B8_OUTPUT"])',
    ...
    $env:B8_OUTPUT = $output
    Invoke-NativeChecked -FilePath $python -ArgumentList @($sheetScript) -FailureMessage 'Could not build Blender v8 contact sheet'
    ...
    if (-not (Test-Path -LiteralPath $contactSheet) -or (Get-Item -LiteralPath $contactSheet).Length -lt 100000) {
        throw 'Blender v8 contact sheet is missing or implausibly small'
    }
}
finally {
...
if ($env:GITHUB_ENV) {
    "B8_OUTPUT=$output" | Add-Content -LiteralPath $env:GITHUB_ENV
}
Write-Host "BLENDER_FACEVERSE_V8_OUTPUT=$output"
Write-Host 'BLENDER_FACEVERSE_V8_RUNNER=PROVEN'
exit 0
