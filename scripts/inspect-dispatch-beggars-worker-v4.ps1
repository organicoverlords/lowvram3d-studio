[CmdletBinding()]
param(
    [int]$RegistrationWaitSeconds = 90
)
...
foreach ($run in @($selectedRuns)) {
    ...
    $jobs = @(Get-RunJobs -RunId ([long]$run.id))
    $artifacts = @()
    if ([string]$run.status -eq 'completed') {
        $artifacts = @(Get-RunArtifacts -RunId ([long]$run.id))
    }
    $failedLogTail = @()
    if (
        [string]$run.status -eq 'completed' -and
        [string]$run.conclusion -notin @('success', 'skipped', 'neutral')
    ) {
        $failedLogTail = @(Get-FailedLogTail -RunId ([long]$run.id))
    }
...
elseif ([string]$buildRun.status -ne 'completed') {
    'BUILD_QUEUED_OR_RUNNING'
}
elseif ([string]$buildRun.conclusion -eq 'success') {
...
$reportPath = Join-Path $ReportRoot ('worker-status-v4-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '.json')
$report | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath $reportPath -Encoding utf8

Write-Host 'BEGGARS_WORKER_STATUS_V4: COMPLETE'
