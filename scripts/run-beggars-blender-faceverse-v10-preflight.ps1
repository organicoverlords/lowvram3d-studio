...
$output = "C:\AI\LowVRAM3D-benchmarks\beggars-scene\blender-faceverse-v10-preflight-$runId"
...
foreach ($required in @($python, $source, $model, $checkpoint, $coefficients, $blender)) {
    if (-not (Test-Path -LiteralPath $required)) {
        throw "Blender v10 input missing: $required"
    }
}
...
Invoke-NativeChecked -FilePath $python -ArgumentList @(
    ...
    "import torch; assert torch.__version__.startswith('2.5.1'), torch.__version__; assert torch.cuda.is_available(); print('BLENDER_V10_FACEVERSE_RUNTIME=PROVEN',torch.__version__,torch.cuda.get_device_name(0))"
) -FailureMessage 'Blender v10 FaceVerse runtime validation failed'
...
Invoke-NativeChecked -FilePath $python -ArgumentList @(
    ...
    'blender\build_beggars_meme_scene_faceverse_v10.py'
) -FailureMessage 'Blender v10 sources failed compilation'
...
try {
    ...
    try {
        ...
        Invoke-NativeChecked -FilePath $python -ArgumentList @(
            ...
            '--anchors','21,31,35',
            '--device','auto'
        ) -FailureMessage 'Blender v10 model-space sequence export failed'
        ...
        Invoke-NativeChecked -FilePath $python -ArgumentList @(
            ...
            '--device','auto'
        ) -FailureMessage 'Blender v10 projection-texture export failed'
    }
    finally {
    ...
    foreach ($path in @($sequence, $projection, $projectionImage, $projectionReport)) {
        if (-not (Test-Path -LiteralPath $path) -or (Get-Item -LiteralPath $path).Length -le 0) {
            throw "Blender v10 generated input missing or empty: $path"
        }
    }
    ...
    Invoke-NativeChecked -FilePath $blender -ArgumentList @(
        '--background',
        '--factory-startup',
        '--python','blender\build_beggars_meme_scene_faceverse_v10.py',
        '--',
        '--sequence',$sequence,
        '--config','configs\scene\beggars_banquet_recreation_v1.json',
        '--output-dir',$output,
        '--render-engine','eevee'
    ) -FailureMessage 'Camera-projected Blender v10 preflight failed'
    ...
    foreach ($name in @(
        ...
        'proof_frame_02.png',
        'proof_frame_03.png',
        'projection_blend_070.png',
        'projection_blend_085.png',
        'projection_blend_100.png',
        'scene_receipt.json',
        ...
        if (-not (Test-Path -LiteralPath $path) -or (Get-Item -LiteralPath $path).Length -le 0) {
            throw "Camera-projected Blender v10 output missing or empty: $name"
        }
    }
    Write-Host 'BLENDER_FACEVERSE_V10_SAVE_RELOAD_STILLS=PROVEN'
    ...
    $sheetLines = @(
        ...
        'import os',
        'root = Path(os.environ["B10_OUTPUT"])',
        'names = ["projected_keyframe_031.png", "projection_blend_070.png", "projection_blend_085.png", "projection_blend_100.png", "hero_clean_render.png", "wide_scene_proof.png"]',
        'images = []',
        'for name in names:',
    ...
    $env:B10_OUTPUT = $output
    Invoke-NativeChecked -FilePath $python -ArgumentList @($sheetScript) -FailureMessage 'Could not build Blender v10 contact sheet'
    ...
    if (-not (Test-Path -LiteralPath $contactSheet) -or (Get-Item -LiteralPath $contactSheet).Length -lt 100000) {
        throw 'Blender v10 contact sheet is missing or implausibly small'
    }
}
finally {
...
if ($env:GITHUB_ENV) {
    "B10_OUTPUT=$output" | Add-Content -LiteralPath $env:GITHUB_ENV
}
Write-Host "BLENDER_FACEVERSE_V10_OUTPUT=$output"
Write-Host 'BLENDER_FACEVERSE_V10_RUNNER=PROVEN'
exit 0
