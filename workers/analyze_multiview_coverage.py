...
def main() -> int:
    ...
    for item in views:
        ...
        with np.load(evidence) as e:
            ...
            reports.append({"view_name":name,"camera":item.get("camera",{}),"meaningful_exterior":bool(item.get("meaningful_exterior",name not in {"top","bottom"})),"status":"ANALYZED","visible_triangles":int(np.unique(ids[visible]).size),"visible_pixels":int(visible.sum()),"original_source_coverage_percent":round(float(direct[visible].mean()*100),3),"donor_coverage_percent":round(float(donor[visible].mean()*100),3),"component_prior_coverage_percent":round(float((prior&(visible_lineage&np.uint16(Lineage.GLOBAL_PRIOR)==0))[visible].mean()*100),3),"global_prior_coverage_percent":round(float((visible_lineage&np.uint16(Lineage.GLOBAL_PRIOR)!=0)[visible].mean()*100),3),"forbidden_face_lineage_exposure":int(forbidden.sum()),"protected_face_overlap":int(protected_overlap.sum()),"expected_new_semantic_coverage_percent":round(float((prior[visible]).mean()*100),3)})
