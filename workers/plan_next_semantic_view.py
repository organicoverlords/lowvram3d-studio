...
def candidate_score(row: dict, max_visible: float) -> float:
    """Return the normalized deterministic score for one analyzed camera."""
    scale = float(row.get("visible_pixels", 0)) / max(float(max_visible), 1.0)
    useful = float(row.get("expected_new_semantic_coverage_percent", 0)) / 100.0 * scale
    donor = float(row.get("donor_coverage_percent", 0)) / 100.0 * scale
    prior = (float(row.get("component_prior_coverage_percent", 0)) + float(row.get("global_prior_coverage_percent", 0))) / 100.0 * scale
    overlap = float(row.get("protected_face_overlap", 0)) / max(float(row.get("visible_pixels", 1)), 1.0)
    risk = float(row.get("forbidden_face_lineage_exposure", 0)) / max(float(row.get("visible_pixels", 1)), 1.0)
    redundancy = float(row.get("original_source_coverage_percent", 0)) / 100.0
    return (useful + 0.7 * donor + 0.3 * prior) - 5.0 * overlap - 4.0 * risk - 1.5 * redundancy


def main() -> int:
    p=argparse.ArgumentParser(); p.add_argument("--coverage-analysis",required=True); p.add_argument("--mesh-hash",required=True); p.add_argument("--uv-hash",required=True); p.add_argument("--provenance-hash",required=True); p.add_argument("--output",required=True); p.add_argument("--min-useful-coverage",type=float,default=5.0,help="minimum useful visible coverage in percent") ; args=p.parse_args()
    data=json.loads(Path(args.coverage_analysis).read_text(encoding="utf-8")); candidates=[]
    max_visible=max((float(row.get("visible_pixels",0)) for row in data.get("views",[])),default=1.0)
    for row in data.get("views",[]):
        if row.get("status")!="ANALYZED" or not row.get("meaningful_exterior",True): continue
        candidates.append((candidate_score(row, max_visible), row))
    candidates.sort(key=lambda x:(-x[0],x[1]["view_name"])); out=Path(args.output)
    if not candidates or candidates[0][0] <= args.min_useful_coverage / 100.0:
